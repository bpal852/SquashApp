...
class PlayersValidator(BaseValidator):
    ...
    def validate(self, df: pd.DataFrame) -> ValidationResult:
        ...
        result = ValidationResult(validator_name="PlayersValidator", data_type="players", is_valid=True)
            ...
            if "Team" in df.columns:
                for team in df["Team"].unique():
                    ...
                    if orders != expected_orders:
                        result.add_warning("Order", f'Team "{team}" has non-sequential orders: {orders}')

        if "HKS No." in df.columns:
            # Check for missing HKS numbers
            missing_hks = df["HKS No."].isna().sum()
            if missing_hks > 0:
                result.add_warning("HKS No.", f"{missing_hks} players missing HKS number")
            ...
            if len(valid_hks) > 0:
                ...
                if duplicates > 0:
                    result.add_error("HKS No.", f"{duplicates} duplicate HKS numbers (same player on multiple teams?)")

        if "Ranking" in df.columns:
            missing_ranking = df["Ranking"].isna().sum()
            if missing_ranking > 0:
                result.add_info("Ranking", f"{missing_ranking} players have no ranking")
        ...
        if "Points" in df.columns:
            ...
            if missing_points > 0:
                result.add_info("Points", f"{missing_points} players have no points")
        ...
        if "Player" in df.columns:
            ...
            if duplicates > 0:
                result.add_warning("Player", f"{duplicates} duplicate player names")
                ...
                if len(dup_players) <= 5:  # Only show if not too many
                    result.add_info("Player", f"Duplicated players: {dup_players.tolist()}")
        ...
        if "Team" in df.columns:
            ...
            if len(small_teams) > 0:
                result.add_warning("Team", f"{len(small_teams)} teams have < 3 players")
            ...
            if len(large_teams) > 0:
                result.add_warning(
                    "Team", f"{len(large_teams)} teams have > {max_team_size} players (max for this division)"
                )
