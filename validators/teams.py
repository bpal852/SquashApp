...
class TeamsValidator(BaseValidator):
    ...
    def validate(self, df: pd.DataFrame) -> ValidationResult:
        ...
        if 'Team Name' in df.columns:
            ...
            if duplicates > 0:
                result.add_error('Team Name', 
                    f'Found {duplicates} duplicate team names')
        ...
        if 'Email' in df.columns:
            ...
            if invalid_emails > 0:
                result.add_warning('Email',
                    f'{invalid_emails} emails missing @ symbol')
        
        # Check for empty convenor names
        if 'Convenor' in df.columns:
            empty_convenors = df['Convenor'].isna().sum()
            if empty_convenors > 0:
                result.add_info('Convenor',
                    f'{empty_convenors} teams have no convenor listed')
