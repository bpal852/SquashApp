...
class SchedulesValidator(BaseValidator):
    ...
    REQUIRED_COLUMNS = {"Home Team", "vs", "Away Team", "Venue", "Time", "Result", "Match Week", "Date"}
    ...
    def validate(self, df: pd.DataFrame) -> ValidationResult:
        ...
        result = ValidationResult(validator_name="SchedulesValidator", data_type="schedules", is_valid=True)
        ...
        self._check_row_count_range(df, self.EXPECTED_MIN_MATCHES, self.EXPECTED_MAX_MATCHES, result)
        ...
        if "Home Team" in df.columns and "Away Team" in df.columns:
            ...
            if len(same_team) > 0:
                result.add_error("teams", f"{len(same_team)} matches have same home and away team")
        ...
        if "Result" in df.columns:
            ...
            result.add_info("Result", f"{total_played} matches played, {total_unplayed} upcoming/unplayed")

            if total_played > 0:
                # Check result format
                invalid_results = played_matches[~played_matches["Result"].str.match(r"^\d+-\d+$|^WO$|^CR$", na=False)]
                if len(invalid_results) > 0:
                    result.add_warning("Result", f"{len(invalid_results)} results have unexpected format")
        ...
        if "Date" in df.columns:
            try:
                ...
                if invalid_dates > 0:
                    result.add_warning("Date", f"{invalid_dates} rows have invalid dates")
                ...
                if len(valid_dates) > 0:
                    ...
                    if min_date.year < current_year - 1:
                        result.add_warning("Date", f"Dates start from {min_date.date()} (before season)")

                    if max_date.year > current_year + 1:
                        result.add_warning("Date", f"Dates extend to {max_date.date()} (after season)")
