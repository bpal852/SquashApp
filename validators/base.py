...
class BaseValidator:
    ...
    def _check_required_columns(self, df: pd.DataFrame, required_columns: Set[str], result: ValidationResult) -> bool:
    ...
    def _check_no_null_values(self, df: pd.DataFrame, columns: Set[str], result: ValidationResult) -> bool:
    ...
    def _check_data_types(self, df: pd.DataFrame, expected_types: Dict[str, str], result: ValidationResult) -> bool:
        ...
        for col, expected_type in expected_types.items():
            ...
            if expected_type in type_mapping:
                ...
                if str(actual_type) not in valid_types:
                    result.add_warning(col, f"Expected type {expected_type}, got {actual_type}")
                    all_valid = False

        return all_valid

    def _check_value_range(
        self, df: pd.DataFrame, column: str, min_val: Any, max_val: Any, result: ValidationResult
    ) -> bool:
        ...
        if series.dtype == "object":
            try:
                ...
                result.add_warning(column, f"Column contains non-numeric values, skipping range validation")
                return True
        ...
        if min_val is not None:
            ...
            if below_min > 0:
                result.add_error(column, f"{below_min} values below minimum {min_val}")
                all_valid = False

        if max_val is not None:
            above_max = (series > max_val).sum()
            if above_max > 0:
                result.add_error(column, f"{above_max} values above maximum {max_val}")
                all_valid = False

        return all_valid

    def _check_row_count_range(self, df: pd.DataFrame, min_rows: int, max_rows: int, result: ValidationResult) -> bool:
        """Check that row count is within expected range."""
        row_count = len(df)

        if row_count < min_rows:
            result.add_warning("row_count", f"Row count {row_count} below expected minimum {min_rows}")
            return False

        if row_count > max_rows:
            result.add_warning("row_count", f"Row count {row_count} above expected maximum {max_rows}")
            return False

        result.add_info("row_count", f"Row count {row_count} within expected range [{min_rows}, {max_rows}]")
        return True

    def _check_unique_values(self, df: pd.DataFrame, columns: List[str], result: ValidationResult) -> bool:
    ...
    def _log_result(self, result: ValidationResult):
        ...
        if result.is_valid:
            ...
            self.logger.error(
                f"❌ Validation failed: {result.validator_name} "
