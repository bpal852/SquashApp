...
class SummaryValidator(BaseValidator):
    ...
    def validate(self, df: pd.DataFrame) -> ValidationResult:
        ...
        result = ValidationResult(validator_name="SummaryValidator", data_type="summary", is_valid=True)
        ...
        if all(col in df.columns for col in ["Played", "Won", "Lost"]):
            ...
            if len(inconsistent) > 0:
                result.add_error("consistency", f"{len(inconsistent)} teams have Won+Lost != Played")
                for idx, row in inconsistent.iterrows():
                    result.add_error(
                        "consistency", f"Team '{row['Team']}': {row['Won']}+{row['Lost']} != {row['Played']}"
                    )
