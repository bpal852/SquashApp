...
class RankingValidator(BaseValidator):
    ...
    ALTERNATIVE_COLUMN_MAP = {"Player": "Name of Player", "Games": "Games Played", "W %": "Win Percentage"}
    ...
    def validate(self, df: pd.DataFrame) -> ValidationResult:
        ...
        result = ValidationResult(validator_name="RankingValidator", data_type="ranking", is_valid=True)
        ...
        self._check_row_count_range(df, self.EXPECTED_MIN_PLAYERS, self.EXPECTED_MAX_PLAYERS, result)
        ...
        if "Games Played" in df.columns:
            ...
            if zero_games > 0:
                result.add_info("Games Played", f"{zero_games} players have 0 games (registered but not yet played)")
        ...
        if all(col in df.columns for col in ["Games Played", "Won", "Lost"]):
            ...
            if len(inconsistent) > 0:
                result.add_error("consistency", f"{len(inconsistent)} players have Won+Lost != Games Played")
        ...
        if all(col in df.columns for col in ["Won", "Games Played", "Win Percentage"]):
            ...
            if len(with_games) > 0:
                ...
                if mismatch > 0:
                    result.add_warning("Win Percentage", f"{mismatch} players have incorrect win percentage")
        ...
        if score_cols_present:
            result.add_info("score_columns", f"Score distribution columns present: {score_cols_present}")
            ...
            if "Games Played" in df.columns:
                ...
                if mismatch > 0:
                    result.add_warning(
                        "score_distribution", f"{mismatch} players have score distribution sum != Games Played"
                    )
        ...
        if "Name of Player" in df.columns:
            ...
            if duplicates > 0:
                result.add_warning(
                    "Name of Player", f"{duplicates} duplicate player names (same name, different teams?)"
                )

        # Team distribution
        if "Team" in df.columns:
            team_counts = df["Team"].value_counts()
            result.add_info("Team", f"{team_counts.nunique()} unique teams, avg {team_counts.mean():.1f} players/team")

            # Check for teams with very few players
            small_teams = team_counts[team_counts < 3]
            if len(small_teams) > 0:
                result.add_warning("Team", f"{len(small_teams)} teams have < 3 players: {small_teams.index.tolist()}")
