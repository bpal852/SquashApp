...
class TestCountGamesWon:
    ...
    def test_normal_match(self):
        row = pd.Series({"Rubbers": ["3-1", "1-3", "3-2"], "Overall Score": "2-1"})
        home, away = count_games_won(row)
        assert home == 7  # 3+1+3
        assert away == 6  # 1+3+2

    def test_match_with_wo(self):
        row = pd.Series({"Rubbers": ["3-1", "WO", "3-2"], "Overall Score": "3-0"})
        home, away = count_games_won(row)
        assert home == 9  # 3+3+3 (WO gets allocated 3-0 to winner)
        assert away == 3  # 1+0+2

    def test_match_with_cr(self):
        row = pd.Series({"Rubbers": ["3-1", "CR", "1-3"], "Overall Score": "2-1"})
        home, away = count_games_won(row)
        assert home == 7  # 3+3+1 (CR gets allocated 3-0 to winner)
        assert away == 4  # 1+0+3

    def test_empty_rubbers(self):
        row = pd.Series({"Rubbers": [], "Overall Score": "0-0"})
        home, away = count_games_won(row)
        assert home == 0
        assert away == 0

    def test_missing_rubbers(self):
        row = pd.Series({"Overall Score": "2-1"})
        home, away = count_games_won(row)
        assert home == 0
        assert away == 0

    def test_non_list_rubbers(self):
        row = pd.Series({"Rubbers": "3-1,1-3", "Overall Score": "1-1"})  # string instead of list
        home, away = count_games_won(row)
        assert home == 0
        assert away == 0

    def test_malformed_rubber_scores(self):
        row = pd.Series({"Rubbers": ["3-1", "abc", "1-3"], "Overall Score": "2-1"})
        home, away = count_games_won(row)
        assert home == 4  # 3+1
        assert away == 4  # 1+3
...
class TestCountValidMatches:
    ...
    def test_rubber_index_out_of_range(self):
        df = pd.DataFrame({"Home Team": ["Team A"], "Away Team": ["Team B"], "Rubbers": [["3-1"]]})  # Only one rubber
...
class TestHomeTeamWon:
    ...
    def test_home_wins_by_score(self):
        row = pd.Series({"Home Score": 3, "Away Score": 1, "Home Games Won": 9, "Away Games Won": 5})
        assert home_team_won(row) == "Home"

    def test_away_wins_by_score(self):
        row = pd.Series({"Home Score": 1, "Away Score": 3, "Home Games Won": 5, "Away Games Won": 9})
        assert home_team_won(row) == "Away"

    def test_tie_score_home_wins_games(self):
        row = pd.Series({"Home Score": 2, "Away Score": 2, "Home Games Won": 8, "Away Games Won": 6})
        assert home_team_won(row) == "Home"

    def test_tie_score_away_wins_games(self):
        row = pd.Series({"Home Score": 2, "Away Score": 2, "Home Games Won": 6, "Away Games Won": 8})
        assert home_team_won(row) == "Away"

    def test_complete_tie(self):
        row = pd.Series({"Home Score": 2, "Away Score": 2, "Home Games Won": 7, "Away Games Won": 7})
    ...
    def test_malformed_data(self):
        row = pd.Series({"Home Score": "abc", "Away Score": 2, "Home Games Won": 7, "Away Games Won": 7})
        assert home_team_won(row) == "Ignore"
