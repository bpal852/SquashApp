...
def scrape_ranking_page(
    league_id: str, year: str, session=None
) -> Tuple[Optional[pd.DataFrame], Optional[pd.DataFrame], list, Optional[pd.DataFrame]]:
    """
...
class RankingScraper(BaseScraper):
    ...
    def _create_summary_dataframe(self, df: pd.DataFrame, filtered_df: pd.DataFrame) -> pd.DataFrame:
        ...
        summary_data = {"Team": [], "Most Games": [], "Most Wins": [], "Highest Win Percentage": []}
    ...
    def _get_unbeaten_players(self, filtered_df: pd.DataFrame) -> list:
        ...
        unbeaten_list = (
            filtered_df[filtered_df["Lost"] == 0]
